...
with calc_col1:
    ...
    calc_name = st.text_input("계산할 종목명 또는 티커", placeholder="예: 삼성전자 또는 AAPL", key="calc_input")
...
with calc_col2:
    ...
    if total_investment > 0:
        ...
        if current_price > 0:
            ...
            st.markdown(f"🔎 현재 수익 현황: <span style='color:{status_color}; font-weight:bold;'>{profit_loss:,.2f} ({profit_rate:+.2f}%)</span>", unsafe_allow_html=True)
    else:
        st.info("구매 가격과 보유 수량을 입력하면 계산기가 작동합니다.")

# 5. 섹션: 펀더멘털 및 밸류에이션 분석 (새로 추가된 기능)
if calc_ticker:
    st.divider()
    st.subheader(f"🔍 '{calc_name}' 펀더멘털 및 밸류에이션 분석")
    
    fund_data = get_fundamental_data(calc_ticker)
    
    if fund_data:
        # 기업 개요
        st.markdown(f"**기업명**: {fund_data['longName']} | **섹터**: {fund_data['sector']} | **산업**: {fund_data['industry']}")
        
        f_col1, f_col2, f_col3, f_col4 = st.columns(4)
        
        with f_col1:
            st.write("🏢 **가치 평가 (Valuation)**")
            pe = f"{fund_data['trailingPE']:.2f}" if fund_data['trailingPE'] else "N/A"
            f_pe = f"{fund_data['forwardPE']:.2f}" if fund_data['forwardPE'] else "N/A"
            st.metric("PER (현재)", pe)
            st.metric("PER (선행)", f_pe)
            
        with f_col2:
            st.write("📖 **장부가 가치**")
            pbr = f"{fund_data['priceToBook']:.2f}" if fund_data['priceToBook'] else "N/A"
            eps = f"{fund_data['trailingEps']:.2f}" if fund_data['trailingEps'] else "N/A"
            st.metric("PBR", pbr)
            st.metric("EPS (주당순이익)", eps)
            
        with f_col3:
            st.write("💰 **수익성 및 배당**")
            roe = f"{fund_data['returnOnEquity']*100:.2f}%" if fund_data['returnOnEquity'] else "N/A"
            div = f"{fund_data['dividendYield']*100:.2f}%" if fund_data['dividendYield'] else "0.00%"
            st.metric("ROE (자기자본이익률)", roe)
            st.metric("배당수익률", div)
            
        with f_col4:
            st.write("📉 **시장 규모**")
            m_cap = f"{fund_data['marketCap'] / 100000000:.1f} 억" if calc_ticker.endswith(".KS") or calc_ticker.endswith(".KQ") else f"${fund_data['marketCap'] / 1000000000:.1f} B"
            st.metric("시가총액", m_cap)
            
        st.caption("※ N/A는 해당 종목의 재무 데이터가 공개되지 않았거나 제공되지 않음을 의미합니다.")
    else:
        st.warning("해당 종목의 펀더멘털 데이터를 불러올 수 없습니다.")
...
st.sidebar.info("""
**데이터 분석 안내:**
- **PER/PBR**: 업종 평균과 비교하여 저평가 여부를 판단하세요.
- **ROE**: 기업이 자본을 얼마나 효율적으로 운영하는지 보여줍니다.
- **배당수익률**: 배당주 투자의 경우 주요 지표로 활용하세요.
""")
