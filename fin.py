...
with col_chart:
    ...
    analysis_name = st.selectbox("분석할 종목 선택", [d['name'] for d in display_data] if portfolio else ["삼성전자"])
    ...
    if target_price > 0 and portfolio:
        # 선택한 종목의 평단가 찾기
        my_item = next((item for item in portfolio if item['name'] == analysis_name), None)
        if my_item:
            proj_gain = (target_price - my_item['buy_price']) * my_item['quantity']
            proj_pct = (target_price / my_item['buy_price'] - 1) * 100
